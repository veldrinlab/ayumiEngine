/**
 * File contains definition of Object3D class.
 * @file    Object3D.cpp
...
namespace AyumiEngine
{
	namespace AyumiDestruction
	{
		...
		Object3D::Object3D(Solid* solid)
		{
			...
			for(unsigned i=0;i<verticesPoints.size();i++)
			{
		...
		/**
		 * Class destructor, free allocated memory. Delete mesh data.
		 */
		Object3D::~Object3D()
		{
			delete vertices;
		...
		Face* Object3D::addFace(Vertex* v1, Vertex* v2, Vertex* v3)
		{
			if(!(v1->equals(v2)||v1->equals(v3)||v2->equals(v3)))
			{
				...
				// I should probably use float precision for everything.
				if(true)
				{
		 ...
		 */
		Vertex* Object3D::addVertex(const Vector& pos, int status)
		{
			int i;
			//if already there is an equal vertex, it is not inserted
			Vertex vertex(pos,status);
		 ...
		 */
		float Object3D::computeDistance(Vertex& vertex, Face& face)
		{
		...
		void Object3D::splitFace(int facePos, Segment& segment1, Segment& segment2)
		{
			...
			else if (startType == Segment::FACE && endType == Segment::FACE)
			{
				...
				float dot1 = fabs(segmentVector * vertexVector);
				...
				float dot2 = fabs(segmentVector * vertexVector);
				...
				float dot3 = fabs(segmentVector * vertexVector);
		...
		/**
		 * Method is used to inverts faces classified as INSIDE, making its normals point outside.
