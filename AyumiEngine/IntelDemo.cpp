...
void updateEntity2(SceneEntity* entity, const float elapsedTime)
{
...
{
	isRunning = true;
	EngineInterface::createEngine("Data/Scripts/config.lua",false);
	//EngineInterface::addUpdateTask("performFrustumCulling");
	//EngineInterface::addRenderTask("renderSprites");
	EngineInterface::connectKeyboardAction(Escape,exitIntelDemo);
	...
	*/
	...
	for(int i = 0; i < 100; ++i)
		for(int j = 0; j < 100; ++j)
		{
			...
			box->setEntityPosition(i*10.0f,j*10.0f,0.0f);
			box->setEntityOrientation(-90.0f,0.0f,0.0f);
			box->setFunctionUpdating(updateEntity2);
			EngineInterface::addIndependentToScene(box);
		}

	/*
	SceneEntity* box2 = new SceneEntity("Box2","Box","NormalMapping");
