...
void main()
{
	...
	if(projCoord.w > 0.0)
	{
		...
		//shadow = textureProj(shadowTexture, projCoord);
		//shadow = lookup(vec2(0.0,0.0));
		
		// 8x8 kernel PCF
		for(float y = -3.5 ; y <=3.5 ; y+=1.0)
		    for(float x = -3.5 ; x <=3.5 ; x+=1.0)
			    shadow += textureProj(shadowTexture0, projCoord + vec4(x/1024.0f*projCoord.w,y/1024.0f * projCoord.w,0.05,0.0));
									
		shadow /= 64.0;
		fragColor = fragColor*(shadow+0.2);
	}
}