...
{	
	...
	{
		pos = viewMatrix * vec4(pointLight[i].position, 1.0f);
	...
	{
		pos = viewMatrix * vec4(spotLight[i].position, 1.0f);
		vec3 spotlightPosEye = pos.xyz / pos.w;	
		spotLightDir[i] = (spotlightPosEye - VertexPos) / spotLight[i].range;    
		spotDir[i] = vec3(viewMatrix * vec4(spotLight[i].direction, 0.0f));
	}
